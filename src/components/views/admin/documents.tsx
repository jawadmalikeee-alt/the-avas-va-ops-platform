...
import { FolderOpen, Upload, Lock, FileText, FileImage, FileSpreadsheet, Download, Trash2, File as FileIcon, Eye } from 'lucide-react'
...
export function AdminDocuments() {
  ...
  const [form, setForm] = useState({ title: '', category: 'Reports', isSensitive: false, clientId: '' })
  ...
  const openUpload = () => {
    setForm({ title: '', category: 'Reports', isSensitive: false, clientId: clients[0]?.id ?? '' })
  ...
  return (
    <div className="space-y-5 pb-16 md:pb-6">
      <Header
        title="Documents"
        subtitle={`${items.length} documents across all clients`}
      ...
      <Card className="rounded-2xl border-border shadow-apple p-0 overflow-hidden">
        {loading ? <div className="p-4"><LoadingSkeleton /></div> :
          filtered.length === 0 ? <EmptyState icon={FolderOpen} title="No documents" description="Upload your first document." action={<Button size="sm" className="bg-avas-blue hover:bg-avas-blue-light" onClick={openUpload}><Upload className="h-4 w-4 mr-1.5" />Upload</Button>} /> :
              ...
              <div key={d.id} className="px-4 py-3 flex items-center gap-3 hover:bg-muted/30 group">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-foreground/60 mt-0.5 truncate font-medium">
                    {d.fileName} · {Math.round((d.fileSize ?? 0) / 1024)} KB · {d.client} · {formatRelative(d.uploadedAt)}
                  </div>
                </div>
                ...
                <div className="flex items-center gap-1">
                  ...
                  <Button size="icon" variant="ghost" className="rounded-full hover:bg-avas-blue/10" onClick={() => downloadFile(d)} title="View">
                    <Eye className="h-[18px] w-[18px] text-avas-blue" />
                  </Button>
      ...
      <Dialog open={uploadOpen} onOpenChange={setUploadOpen}>
        <DialogContent className="sm:max-w-[480px] rounded-2xl">
          ...
          <div className="space-y-4">
            <div>
              <Label className="text-[11px] text-foreground/60 font-bold">Client *</Label>
              <Select value={form.clientId} onValueChange={(v) => setForm({ ...form, clientId: v })}>
                <SelectTrigger className="mt-1.5 h-10 text-sm"><SelectValue placeholder="Select client…" /></SelectTrigger>
                <SelectContent>
