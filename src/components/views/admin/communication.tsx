...
import { Avatar, EmptyState, LoadingSkeleton, SectionHeader } from '@/components/ui-primitives'
...
export function AdminCommunication() {
  ...
  return (
    <div className="space-y-5 pb-16 md:pb-6 h-[calc(100vh-7rem)] flex flex-col">
      <Header title="Communication" subtitle="Internal & client messaging" />
        ...
        <Card className="border-border/70 shadow-none flex flex-col min-h-0">
          ...
          <div className="flex-1 overflow-y-auto">
            {loading ? <LoadingSkeleton rows={4} /> :
              partners.length === 0 ? <EmptyState icon={MessageSquare} title="No conversations" /> :
              partners.map((p) => (
                <button
                  key={p.id}
                  onClick={() => setActivePartner(p.id)}
                  className={`w-full px-3 py-2.5 flex items-center gap-2.5 text-left border-b border-border/50 hover:bg-muted/30 ${activePartner === p.id ? 'bg-muted/50' : ''}`}
                >
          ...
          {!activePartner ? (
            ...
            <>
              ...
              <div className="px-3 py-2 border-t border-border flex items-center gap-2">
                ...
                />
                <Button size="sm" className="h-8" onClick={send} disabled={sending || !input.trim()}>
